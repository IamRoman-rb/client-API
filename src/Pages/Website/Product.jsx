...
const Product = () => {
  ...
  useEffect(() => {
    const fetchRelated = async () => {
      ...
      try {
        const res = await fetch(
          `/api/productos/categoria/${producto.categoria.id}`
        );
        if (!res.ok) return setRelated([]);
        const data = await res.json();
        setRelated(
          Array.isArray(data) ? data.filter((p) => p.id !== producto.id) : []
        );
      } catch (e) {
        setRelated([]);
      } finally {
  ...
  const agregarAlCarrito = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/compras/carrito/agregar", {
        method: "POST",
        headers: {
          ...
          {puedeComprar() && (
            <div className={Style.cartActions}>
              <div className={Style.quantitySelector}>
                ...
                >
                  {[...Array(Math.min(10, producto.cantidad || 1))].map(
                    (_, index) => (
          ...
          <ul className={Style.relatedProductsList}>
            {related.map((r) => (
              <li
                key={r.id}
                onClick={() =>
                  navigate(`/productos/${encodeURIComponent(r.nombre)}`)
                }
                style={{
                  cursor: "pointer",
                  display: "flex",
