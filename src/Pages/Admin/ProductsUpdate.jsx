...
const ProductsUpdate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/productos/editar/${producto.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${user.accessToken}`,
  ...
  if (loading)
    return <div className={Style.mainProduct}>Cargando producto...</div>;
  if (error) return <div className={Style.mainProduct}>Error: {error}</div>;
  if (!producto)
    return <div className={Style.mainProduct}>Producto no encontrado</div>;
